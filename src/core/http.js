	...
	if( config.web.authentication.enable ) {
		...
		server.use(function (req, res, next) {
			...
			var auth = req.headers['authorization'],
				invalidCredentials = (!config.web.authentication.users[req.username] || hash(req.authorization.basic.password) !== config.web.authentication.users[req.username].password);

			// Log invalid login attempts
			if (auth && invalidCredentials) {
